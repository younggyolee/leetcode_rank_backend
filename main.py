...
@app.post('/rating/')
def update_rating(db: Session = Depends(get_db)):
    usernames = get_usernames(db)
    for username in usernames:
        try:
            fetched_data = get_data_from_leetcode(username)
            update_user(db, username, fetched_data['rating'], fetched_data['topPercentage'])
